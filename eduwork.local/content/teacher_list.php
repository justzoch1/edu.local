    ...
    <div class="row mb-3">
        <div class="col-md-6 offset-md-3">
            <form action="" method="GET" class="form-inline justify-content-center">
                ...
                <a href="export_excel.php?search=<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>" class="btn btn-success ">Скачать в EXCEL</a>
            </form>
        </div>
    </div>
    <table class="table">
        ...
        <tbody>
            ...
            if (isset($_GET['search'])) {
                $search = $_GET['search'];
                $stmt = $conn->prepare("SELECT * FROM teachers WHERE role = 'teacher' AND (last_name LIKE ? OR first_name LIKE ? OR middle_name LIKE ? OR group_number LIKE ?)");
                $searchParam = "%$search%";
